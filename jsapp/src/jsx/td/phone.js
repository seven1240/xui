...
import { Modal, Form, FormGroup, Col, ControlLabel, NavItem,  Button } from 'react-bootstrap';
...
class CallPopup extends React.Component {
	...
	handleAnswer() {
		...
		if (this.props.curCall) {
			this.props.curCall.answer({
	...
	handleHangup() {
		if (this.props.curCall) {
			this.props.curCall.hangup();
		}
	}

	render() {
		const props = Object.assign({}, this.props);
		const curCall = props.curCall;
		delete props.curCall;
		...
		if (curCall) {
			const call_params = curCall.params;
			callerName = call_params.caller_id_name;
			callerNumber = call_params.caller_id_number;
			destination_number = call_params.callee_id_number ? call_params.callee_id_number : localStorage.getItem('xui.username');
		}

		return <Modal {...props} aria-labelledby="contained-modal-title-lg" dialogClassName="xmodal">
			<Modal.Header closeButton>
				<Modal.Title id="contained-modal-title-lg"><T.span text={T.translate("New Call")}/></Modal.Title>
			</Modal.Header>
			<Modal.Body>
				<Form horizontal id="callPopup">
					<h2 className="modal-title">
						<span style={{color: "red"}}><T.span text={T.translate("Incoming Call") + ":"} style={{color: "black"}}/>&nbsp;
							{callerName + " <" + callerNumber + "> -----> " + destination_number}
						</span>
					</h2>
				</Form>
			</Modal.Body>
			<Modal.Footer>
				<Button bsStyle="success" bsSize="xsmall" onClick={this.handleAnswer.bind(this)}>
					<i className="fa fa-phone" aria-hidden="true"></i>&nbsp;
					<T.span text={T.translate("Answer Call")}/>
				</Button>
				<Button bsStyle="danger" bsSize="xsmall" onClick={this.handleHangup.bind(this)}>
					<i className="fa fa-phone" aria-hidden="true"></i>&nbsp;
					<T.span text={T.translate("Refuse Call")}/>
				</Button>
			</Modal.Footer>
		</Modal>;
	}
...
class Phone extends React.Component {
	...
	handleVertoDialogState (e) {
		...
		if (d.state.name == "ringing" && d.direction.name == "inbound" && d.params.callee_id_number == localStorage.getItem('xui.username')) {
			this.setState({popupShow: true});
		}

		if (d.state.name == "hangup" && this.state.popupShow) {
			this.setState({popupShow: false});
		}

		if (d.state.name == "active" && this.state.popupShow) {
			this.setState({popupShow: false});
		}
	...
	render() {
		...
		var popupClose = () => this.setState({ popupShow: false });
		...
		return <NavItem eventKey="phone">
			<div className="hgt">
				...
				<CallPopup show={this.state.popupShow} onHide={popupClose} curCall={this.state.curCall}/>
		</NavItem>
	}
};
