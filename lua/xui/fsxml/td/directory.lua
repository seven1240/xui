...
if not params and not env then
...
if params then
	...
	invalid_user = params:getHeader("invalid_user")
elseif env then
	user = env:getHeader("user")
	domain = env:getHeader("domain")
	purpose = env:getHeader("purpose")
	action = env:getHeader("action")
	invalid_user = env:getHeader("invalid_user")
end

function get_invalid_user()
	freeswitch.consoleLog("debug", "registered users have reached the max!!!\n")
	...
	pars = '<param name="password" value="xyt123qwe*#!"/>' ..
			'<param name="vm-password" value="xyt123qwe*#!"/>' ..
			'<param name="dial-string" value="' .. dial_string .. '"/>'

	vars =  '<variable name="user_context" value="default"/>' ..
				'<variable name="accountcode" value="invalid_user"/>' ..
				'<variable name="caller_id_name" value="invalid_user"/>' ..
				'<variable name="effective_caller_id_name" value="invalid_user"/>' ..
				'<variable name="effective_caller_id_number" value="invalid_user"/>'
...
if invalid_user ~= nil then
	return get_invalid_user()
end
