...
local actions_table = {}
local sql = "SELECT * FROM routes WHERE context = '" .. context .. "' AND max_length >= " .. string.len(dest) .. " AND " .. escape(dest) .. " LIKE prefix || '%' ORDER BY length(prefix) DESC, max_length DESC LIMIT 1"
local found = false
