...
def getMemorystate():
  ...
  line = {'percent': phymem.percent, 'used': phymem.used/1024/1024, 'total': phymem.total/1024/1024}
  return line
  # line = "%5s%% %6s/%s"%(
  #   phymem.percent,
...
class SimpleWebSocket(WebSocket):
    ...
    def handleMessage(self):
        data = {'cpu': getCPUstate(), 'memory': getMemorystate()}
        ret = json.dumps(data).decode("utf-8")
        print ret
...
server = SimpleWebSocketServer('', 8000, SimpleWebSocket)
server.serveforever()
